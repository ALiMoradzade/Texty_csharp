...
namespace Texty.Encoding
{
    internal class TextEncodingConverter
    {
        ...
        public TextEncodingConverter(bool isBigEndian = true, bool bom = false, string separator = " ")
        {
        ...
        {
            this.text = text;
            utf8 = string.Join(separator, EncodeToUTF8(text));
        ...
        public void Decode(string code, Encoding codeEncoding)
        {
            ...
            {
                text = DecodeToUTF8(splittedUnicodes);
            }
            else if (codeEncoding == Encoding.UTF16)
            {
                text = DecodeToUTF16(splittedUnicodes, isBigEndian, BOM);
            }
            else if (codeEncoding == Encoding.UTF32)
            {
                text = DecodeToUTF32(splittedUnicodes, isBigEndian, BOM);
            }
        ...
        {
            string[] removeHexaPrefix = hexadecimalCodes.Select(x => x.Replace("0x", "")).ToArray();
